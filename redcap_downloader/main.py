...
def main():
    ...
    output_folder = os.path.join(f"{properties.download_folder}_{datetime.today().strftime('%Y-%m-%d')}")
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(output_folder, "download.log")),  # Log to a file
